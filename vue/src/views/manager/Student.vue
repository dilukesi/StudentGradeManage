<template>
  <div>
    <div class="card" style="margin-bottom:18px ">
      <el-input v-model="data.username" style="width: 280px;margin-right: 10px" placeholder="请输入学生账号查询" :prefix-icon="Search"/>
      <el-input v-model="data.name" style="width: 280px;margin-right: 10px" placeholder="请输入学生名称查询" :prefix-icon="Search"/>
  ...
  <div class="card " style="margin-bottom:18px;">
    ...
    <el-button type="primary" round @click="handleBatchAdd">批量新增</el-button>
    </div>
    <div>
...
<script setup>
  ...
  const del =(id)=>{
    ElMessageBox.confirm('删除数据后无法回复，您确认删除吗？','删除确认',{type:'warning'}).then(_res=> {
...
const handleBatchAdd = () => {
  const numberOfStudents = 120; // 要生成的学生数量
  const students = [];
  ...
  for (let i = 0; i< numberOfStudents; i++) {
    const student = {
      ...
      password: generateFixedPassword(),
      name: "",
      phone: "待完善",
      email: "待完善",
      sex: "待完善",
  ...
  // 调用后端 API 进行批量新增
  request.post(baseurl + "/batchAdd", students).then((res) => {
    if (res.code === "200") {
...
const generateRandomUserid = () => {
    const min = 20210001; // 范围的最小值
    const max = 20211000; // 范围的最大值
    const randomId = Math.floor(Math.random() * (max - min + 1) + min);
       return randomId;
};
const generateRandomUsername = () => {
    const randomNumber = Math.floor(Math.random() * (1000 - 1 + 1)) + 1; // 生成 1 到 10000 之间的随机数
    return `test${randomNumber}`;
};

const generateFixedPassword = () => {
    return "123456";
};
